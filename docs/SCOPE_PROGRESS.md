...
**🔴 収益拡大（自動化で殴れる候補・2026-04-28再評価）**
2026-04-28の構造的反省を経て再整理。「ツール完成→収益0」のFIGHT PREDICTパターンは「配布チャネル無し」が根本原因と特定。**配布が組み込まれた選択肢のみを「有効候補」とし、それ以外（A・C・E・F）は配布側を解決しない限り着手しない方針**。現在ユーザーは現状維持を選択、いずれの候補にも未着手。
...
- [ ] **G. YouTube Shorts自動生成・自動投稿**（最有力候補・配布アルゴリズム組込）— GitHub Actionsで毎朝動画自動生成（VOICEVOX 無料TTS＋ffmpegでstat可視化動画＋YouTube Data API投稿）。著作権はデータ可視化のみで回避。**配布はYouTube algorithmが担当**＝新参チャンネルにも視聴者を運ぶ仕組みが組み込まれている数少ない無料路線。初期セットアップ30分（YouTubeチャンネル開設＋OAuth認可）が必要、以降は完全自動。工数3〜5日。期待値：現実1,000〜5,000再生／月・¥500〜2,000、楽観1〜5万再生／¥3,000〜10,000。
- [ ] **H. Bluesky自動投稿**（保険的選択肢・低コスト）— Bluesky API完全無料・無制限。X Botコードを30行書き換えで動く。工数1〜2時間。ただし日本語MMAクラスタが小規模で再生数期待値は低い。Gで結果が出ない時の反応見の試金石として位置付け。
- [ ] **A. プログラマティックSEO**（着手見送り・期待値低）— ドメインオーソリティ不足で長尾検索もWiki/Sherdog/Tapology等に押し負ける。実装しても6ヶ月後に月数百〜数千PVの可能性、その間に12/31キャッシュポイント通過。配布ボトルネックを解決しない限り徒労。
- [ ] **D. Discord Bot配布**（手動アウトリーチ要・着手見送り）— サーバー招待依頼のDM5〜10通が必要、ユーザー判断でNG。bot実装自体は1〜2日で可能だが配布されない以上意味なし。
- [ ] **C. note戦略を事前→事後に転換**（着手見送り）— 「[大会名] 結果 AI予測の答え合わせ」型に切替＋GitHub Actionsからnote下書き自動生成。ただしnote公開作業はユーザー手動必須＋自前流入無しで0いいね問題は変わらず。
- [ ] **E. 的中ログの物語化**（着手見送り・配布無しでは意味なし）— `report_generator.py` 改良で機械的出力→物語型コピー化。投稿先（X／note）の配布が解決しない限り効果なし。
- [ ] **F. スレッド先出し→note回収**（B凍結により実質凍結）— Bが復活しない限り運用できない。
...
**⚪ 当面保留（ユーザー判断で手動作業を行わない方針）**
- [ ] Xのbio・固定ツイートを実際に更新（2026-04-27判断で保留：現状bioが新案ABCとほぼ同等の訴求になっており、固定ツイート画像化のみ要だが手動作業ROIを優先しスキップ。素材は `marketing/x_profile_drafts.md` ＋ `marketing/pinned_tweet.png` で温存、気が向いたら反映可）
- [ ] 次大会の事前note記事公開（UFC FN 275等。前回274が0いいねで、現状のやり方では量産しても無意味と判断。Cの事後note戦略へ転換予定）
- [ ] 既存格闘技クラスタへの手動エンゲージ開始（playbook `marketing/engagement_playbook.md` は完成しているが、手動運用負荷が継続せず実質未着手）
...
- 2026-04-19: X日次運用フェーズに移行。毎朝8時JSTに届くGmailから1投稿ずつコピペで運用。7日前→main、6日前〜前日→cardローテーション、翌日→的中ログ。目標は3ヶ月継続でフォロワー500〜1000、副業月5,000〜15,000円。
- 2026-04-25: 的中ログ自動化稼働。従来の「翌日は手動集計」を廃止、Sherdog結果スクレイプ＋予測自動解決で完全自動化（`x-hit-log.yml` 毎朝8:15 JST）。Gmail下書きメールの流れは `7日前=main / 6〜1日前=card / 当日=main / 翌日=hit-log` に4系統で運用される。手動エンゲージplaybook（`marketing/engagement_playbook.md`）も完成し、朝のGmail確認＋昼夜のリプ・引用RTが運用動線として確立。
- キャッシュポイント: 12/31 RIZIN大晦日と9/10 Super Rizin 5 で収益爆発を狙う（ABEMA PPVアフィの成約単価が高い）。
...
- 2026-04-28: B（X Bot化）凍結判断。コード／Secrets／PAT認証フローまで全部通したが、最終テストで X API status=402 CreditsDepleted。新規開発者アカウントは Free tier 廃止＝Pay Per Use（最小チャージ要）のみで、ユーザー判断で「無料じゃないなら諦める」。`X_BOT_ENABLED=false` で凍結、コードは温存（API無料化／代替プラットフォーム移行時に流用可能）。
- 2026-04-28: 構造的反省。ユーザーから「お前いろんなツール作ってきたけど最終的には全部こうなるやん」の指摘を受けて、「ツール完成→収益0」が偶然ではなく構造的パターンであると整理。AIが得意なのは「作る」だが収益化は「届ける」で決まる、という非対称性が原因。これまで build-side の提案（次の機能・自動化・SEO）を出し続けて毎回 distribute-side の壁で崩壊してきた。今後は新ツール／機能の提案前に「配布はどうする？」を必ず確認する方針に転換。配布チャネルが組み込まれた選択肢（Chrome拡張・YouTube Shorts自動投稿・既存コミュニティ向けニッチツール等）のみを有効候補とする。現状の残タスクA・C・E・Fは配布が解決しない限り着手しない（徒労防止）。
- 2026-04-28: 残タスク再評価。ユーザーが「無料自動化なら折れる」と表明し、新候補G（YouTube Shorts自動生成・自動投稿）を追加。VOICEVOX無料TTS＋ffmpegでstat可視化動画＋YouTube Data API投稿。YouTube algorithmが配布を担うため、これまでで唯一「無料・自動・配布チャネル組込み」が揃う候補。同時にH（Bluesky自動投稿）を低コスト保険的選択肢として追加。ただし本日は判断見送り、現状維持で終了。AdSense審査結果＋12/31大晦日キャッシュポイント待ち、いずれか着手したくなったらG優先で再開。
